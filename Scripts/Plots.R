...
plotHighestCor <- function(dataset, flip = F){
  ...
  # par(mfrow = c(1,2))
  p1 <- ggplot(numdata, aes(x=numdata[[max_row]], y=numdata[[max_col]])) + geom_point() + geom_smooth() + labs(x = max_row, y = max_col)
  p2 <- ggplot(numdata, aes(x=numdata[[max_col]], y=numdata[[max_row]])) + geom_point() + geom_smooth() + labs(x = max_col, y = max_row)
  p1+p2
}